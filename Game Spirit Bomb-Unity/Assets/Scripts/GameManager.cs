...
public class GameManager : MonoBehaviour
{
    ...
    public bool CheckGrid(Vector2Int pos){
        if(pos.x < 0||pos.x>=RowCount||pos.y<0||pos.y>=RowCount) return false;
        return true;
    }
    ...
    protected bool CheckWinState(){
        ...
        if(CheckGrid(VirusRed.index + right)) possibleSteps.Add(right);
        if(CheckGrid(VirusRed.index + up))    possibleSteps.Add(up);
        if(CheckGrid(VirusRed.index + left))  possibleSteps.Add(left);
        if(CheckGrid(VirusRed.index + down))  possibleSteps.Add(down);

        //检查周围的区域是否都有绿色格子，如果都有为真，否则为假。
        for(int i=0; i<possibleSteps.Count; i++){
            if(!IfPosGreen(VirusRed.index + possibleSteps[i])) return false;
        }
